%macro T0_SuppressSecondaryRow(Data=,
							   VarList=,
							   Obs=,
							   SuppressValue=-1
                              );
/**********************************************************************************
 Macro Name:		T0_SuppressSecondaryRow.sas
 Programmer:		CKR	
 Project:			NATS 2021
 Created:           June 2022
  ...
  data &Data;
	...
	do;  /* PERFORM SECONDARY SUPPRESSION ON THIS, THE DESIGNATED, ROW */
	  ...
	  %do i=1 %to &numGroup;
	    ...
	    /* SUPPRESSED VALUES >5 THEN SECONDARY SUPPRESSION NOT NEEDED.   */
	    if %scan(&&Group&i,1)=&SuppressValue then 
        do;
          SuppressNum+1;
	      SuppressTot+__&Var;  /* __&Var=ORIGINAL VALUE RETAINED FROM PRIMARY SUPPRESSION */
	    end;
	  %end;

	  if SuppressNum=1 or (SuppressNum>1 and 1<=SuppressTot<=5) then
	  do;   
	    /* FIND NEXT LOWEST N>5 TO SUPPRESS (IF ONE EXISTS) */
	    LowestN=10000000000;  /* ARBITRARY LARGE NUMBER */
		LowestIndex=0;
        %do i=1 %to &numGroup;
          if _Group&i(1)>5 and _Group&i(1)<LowestN then
		  do;
		    LowestN=_Group&i(1);
			LowestIndex=&i;
		  end;
